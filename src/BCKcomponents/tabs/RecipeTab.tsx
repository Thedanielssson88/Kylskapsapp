...
export const RecipeTab = ({ t, setActiveTab, setChatInput, setAttachedRecipe }: { t: any, setActiveTab: any, setChatInput: any, setAttachedRecipe: any }) => {
    ...
    React.useEffect(() => {
        ...
        return (
            ...
            {(recipeView === 'generated' ? generatedRecipes : savedRecipes).map((recipe, idx) => {
                ...
                return (
                    <div key={idx} onClick={() => setViewingRecipe(recipe)} className={clsx("rounded-2xl overflow-hidden shadow-sm border relative cursor-pointer hover:shadow-md transition-shadow", t.cardBg, t.border)}>
            ...
            {viewingRecipe && (() => {
                ...
                return (
                    ...
                    <button onClick={() => setViewingRecipe(null)} className="p-1 hover:bg-white/20 rounded-full transition-colors"><X className="w-5 h-5" /></button>
                    </div>
                    ...
                    <div className="flex gap-2">
                    <button onClick={() => { setAttachedRecipe(viewingRecipe); setActiveTab('chat'); setViewingRecipe(null); setChatInput(`Jag har en fråga om receptet "${viewingRecipe.title}": `); }} className="flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white"><MessageCircle className="w-5 h-5" /> Fråga Kocken</button>
                    <button onClick={() => setShowScheduleDialog(true)} className="flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white"><Calendar className="w-5 h-5" /> Veckoschema</button>
                    </div>
                    <div className="flex gap-2">
                    <button onClick={() => { toggleSavedRecipe(viewingRecipe); setViewingRecipe(null); }} className={clsx("flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2", isSaved ? "bg-red-500 text-white" : "bg-purple-500 text-white")}><Heart className="w-5 h-5" fill={isSaved ? "currentColor" : "none"} /> {isSaved ? "Ta bort" : "Spara"}</button>
                    <button onClick={() => setViewingRecipe(null)} className="px-6 py-3 bg-gray-200 dark:bg-gray-700 rounded-xl font-bold">Stäng</button>
                    </div>
                    </div>
                    </div>
                    </div>
            ...
            {showScheduleDialog && viewingRecipe && (
                ...
                <button onClick={() => setShowScheduleDialog(false)} className="p-1"><X className="w-5 h-5" /></button>
                </div>
                </div>
                ...
                <button onClick={() => {
                    ...
                    setShowScheduleDialog(false); setViewingRecipe(null); setActiveTab('mealplan');
